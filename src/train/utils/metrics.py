...
def fast_hist(label_pred, label_true, num_classes):
    hist = np.bincount(num_classes * label_true.astype(int) +
                       label_pred, minlength=num_classes ** 2)
...
class AccTopk():
    ...
    def update(self, predict, target):
        ...
        self.top5_correct += np.sum(predict[background_idx]
                                    == target[background_idx])
        not_background_idx = np.logical_not(background_idx)
        self.top5_correct += np.sum(np.absolute(
            predict[not_background_idx] - target[not_background_idx]) < self.k)
...
def update_metrics(metric_dict, pair_data):
    ...
    for i in range(len(metric_dict['name'])):
        ...
        if predict is None:
            predict, target = converter(
                pair_data[data_src[0]]), converter(pair_data[data_src[1]])
