...
def calc_k(line):
    ...
    line_x = line[::2]
    line_y = line[1::2]
    length = np.sqrt((line_x[0] - line_x[-1]) ** 2 +
                     (line_y[0] - line_y[-1]) ** 2)
    if length < 90:
...
def draw(im, line, idx, show=False):
    ...
    for i in range(len(line_x) - 1):
        cv2.line(im, pt0, (int(line_x[i + 1]),
                 int(line_y[i + 1])), (idx,), thickness=16)
...
def get_tusimple_list(root, label_list):
    ...
    for i in range(len(lanes)):
        ...
        for j in range(len(lanes[i])):
            ...
            valid = lanes[i][j] != -2
            line_txt_tmp = [None] * \
                (len(h_samples[i][valid]) + len(lanes[i][j][valid]))
...
def generate_segmentation_and_train_list(root, line_txt, names):
    ...
    for i in tqdm.tqdm(range(len(line_txt))):
        ...
        k_neg = ks[ks < 0].copy()
        k_pos = ks[ks > 0].copy()
        # -10 means the lane is too short and is discarded
...
def get_args():
    ...
    parser.add_argument('--test_files', required=False,
                        help='comma seperated list of json files containing test data')
    return parser
...
#     with open(os.path.join(args.root, args.test_files[:-4] + 'txt'), 'w') as fp:
#         for name in names:
#             fp.write(name + '\n')

if __name__ == "__main__":
    train_file = ['train_labels.json']
    # training set
    names, line_txt = get_tusimple_list(config.data_root, train_file)
    # generate segmentation and training list for training
    generate_segmentation_and_train_list(config.data_root, line_txt, names)

    test_file = ['test.json']
    # testing set
    names, line_txt = get_tusimple_list(config.data_root, test_file)
    # generate testing set for testing
    with open(os.path.join(config.data_root, 'test.txt'), 'w') as fp:
        for name in names:
            fp.write(name + '\n')
    ...
    names, line_txt = get_tusimple_list(config.data_root, test_file)
    # generate testing set for testing
    with open(os.path.join(config.data_root, 'validate.txt'), 'w') as fp:
        for name in names:
            fp.write(name + '\n')
